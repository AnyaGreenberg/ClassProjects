//gcc -Wall -o hrdna_KS1 HRDNA_Stephan.c -lgsl -lgslcblas -lm
//./hrdna_KS1 $popsize_2N $iniCN $selCoeff $time_g $trial $i
...
int main(int argc, char* argv[]) {
    ...
    if (ini_CN < CN_LLimit || ini_CN > CN_ULimit) {
        ...
        //randomize exchange rate per simulation
	exchg_r = -1;
	while (exchg_r > 1 || exchg_r < 0) {
        	exchg_r = gsl_ran_gamma(r,2,10);
	}
        printf("%d\t%lu\t%lu\t%lu\t%lf\t%lu\t%lf\t", sim, my_seed, popsize_2N, ini_CN, sel_coeff, time_g, exchg_r);

        //print gen, nCN, and nIndiv to outFile1
        sprintf(fName, "HRDNA_Stephan_dist_popsize_2N%d_iniCN%d_selCoeff%.7lf_gen%d_trial%d_sim%d.txt", popsize_2N, ini_CN, sel_coeff, time_g, trial, sim);
        outFile1 = fopen(fName, "w"); 
        fputs("gen\tnCN\tnIndiv\n", outFile1);
...
void Setup_CNgroup(FILE* outF1) {
    ...
    group[0].nCN = ini_CN;
    group[0].nIndiv_o = popsize_2N; //o vs. n???
    group[0].nIndiv_n = 0;
    ...
    do {
        ...
        if (i1 != i2) {   
            ...
            if (gsl_rng_uniform(r) < exchg_r){ //if a random number is less than exchg_r, accept exchange       
                ...
                if (a < 0.5) n_CN = n_CN1; //50% chance for each parent
                else n_CN = n_CN2;
                
                CNgroup_new(n_CN); //add create new CN group if it is within limit bounds
            }
