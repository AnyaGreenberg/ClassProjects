<?php
require_once("../db_functions.php");
session_id("adopter");
session_start();

if ( ! empty( $_POST ) ) {
    $conn = database_connect();

    $query = "SELECT Email, Password, Ssn FROM USERS NATURAL JOIN ADOPTER_EMAIL NATURAL JOIN ADOPTER WHERE User_type = 'ADOPTER' AND Email = '" . $_POST['email'] . "';";
    //echo $query;
    $qry = $conn->prepare($query);
    $qry->execute();
    if ($qry->errorCode() != 0) {
        print_r($qry->errorInfo());
    }
    $result = $qry->fetchAll(PDO::FETCH_ASSOC);
    //echo $result[0];

    if(sizeof($result) == 1) {
        if(hash_function($_POST['password']) == $result[0]['Password']) {
            $_SESSION['user_id'] = $result[0][Email];
            $_SESSION['user_ssn'] = $result[0][Ssn];
            header("Location: viewAdopterRequests.php");
        }else{
            echo "Wrong email and password combination..";
        }
    }else{
        echo "Wrong email and password combination.";
    }
}

?>

<html>
<body>

<head>
<style>
ul {
  list-style-type: none;
  margin: 0;
  padding: 0;
}

li {
  display: inline;
}
</style>
</head>

<body>
     <h1> Adopter Center </h1>
<ul>
    <li><a href="signup.php">Sign up</a></li> <br>
	<li><a href="../home.php">Return to Homepage</a></li>

</ul>

<h2> Login  </h2>

<form action="" method="post">
    <input type="text" name="email" placeholder="Enter your email" required>
    <input type="password" name="password" placeholder="Enter your password" required>
    <input type="submit" value="Submit">
</form>

</body>
</html>
